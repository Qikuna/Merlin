...
def input_parrafo(stdscr, y, x, max_width, max_lines):
    ...
    while True:
        lines = [
            buffer_texto[i : i + max_width]
            for i in range(0, len(buffer_texto), max_width)
        ]
        if not lines:
            lines = [""]

        for l_idx in range(max_lines):
            safe_addstr(stdscr, y + l_idx, x, " " * max_width)

        for l_idx, line_text in enumerate(lines[:max_lines]):
            safe_addstr(stdscr, y + l_idx, x, line_text)

        total_chars = len(buffer_texto)
        current_line_idx = total_chars // max_width

        if current_line_idx >= max_lines:
            current_line_idx = max_lines - 1
            cursor_char_idx = max_width - 1
        else:
            cursor_char_idx = total_chars % max_width

        try:
            stdscr.move(y + current_line_idx, x + cursor_char_idx)
        except curses.error:
            pass
        stdscr.refresh()
        ...
        if ch in ("\n", "\r", 10, 13) or ch == curses.KEY_ENTER:
            break

        elif ch in ("\x7f", "\x08", b"\x7f", curses.KEY_BACKSPACE):
            if len(buffer_texto) > 0:
                buffer_texto = buffer_texto[:-1]

        elif isinstance(ch, str):
            if len(buffer_texto) < (max_width * max_lines) - 1:
                if ord(ch) >= 32:
                    buffer_texto += ch
...
def tui_controller(stdscr):
    ...
    while True:
        ...
        for i, block in enumerate(blocks):
            ...
            if max_text_width > 0:
                text_lines = [
                    clean_text[j : j + max_text_width]
                    for j in range(0, len(clean_text), max_text_width)
                ]
            else:
        ...
        if key == ord("q") or key == 27:
            ...
            safe_addstr(
                ...
                3,
                25,
                f"[{start_time}] Write (Enter to save):",
                curses.A_BOLD,
            )
            ...
            if focus == "history":
                ...
                safe_addstr(
                    ...
                    height - 2,
                    25,
                    f"Delete ALL data for {date_to_delete}? (y/n): ",
                    curses.A_BOLD,
                )
                stdscr.refresh()
